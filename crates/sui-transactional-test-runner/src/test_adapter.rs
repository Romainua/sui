...
impl<'a> SuiTestAdapter<'a> {
    fn upgrade_package(
        ...
        let current_package_id = (*modules[0].1.self_id().address()).into();
        ...
        let digest: Vec<u8> =
            MovePackage::compute_digest_for_modules_and_deps(&all_bytes, &dependencies).into();
        ...
        let transaction = self.sign_txn(Some("A".to_owned()), data); // FIXME: unhardcode sender
        let summary = self.execute_txn(transaction, gas_budget)?;
